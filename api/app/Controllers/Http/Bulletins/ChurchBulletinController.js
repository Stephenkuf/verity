...
class ChurchBulletinController {
  async createChurchBulletin({ request, response, auth }) {
    try {
      let userDenomination, LogchurchBulletin;
      const {
        bulletin_type,
        bulletin_subject,
        bulletin_body,
      ...
      if (bulletin_type == "denomination") {
        userDenomination = await denominationInfo
          .query()
        ...
        LogchurchBulletin = await churchBulletinLog.findOrCreate({
  ...
  async viewChurchBulletin({ request, response, auth }) {
    try {
      let is_branch, churchBulletin, userDetails;
      const { user } = auth.current;
      ...
      if (is_denomination) {
        churchBulletin = await Database.table("bulletin_users")
      ...
      if (!is_denomination || is_denomination == null) {
        ...
        if (is_branch || is_branch != null) {
          churchBulletin = await Database.table("bulletin_users")
            .leftJoin("church_bulletins", "bulletin_id", "church_bulletins.id")
            .where(function () {
              this.where("branch_id", is_branch.id).orWhere(
                "denomination_id",
                is_branch.denomination_id
              );
            })
            .andWhere(function () {
        ...
        if (!is_branch) {
          userDetails = await additionalUserInfo
            .query()
            .where("user_id", user.id)
            .first();
          ...
          churchBulletin = await Database.table("bulletin_users")
            .leftJoin("church_bulletins", "bulletin_id", "church_bulletins.id")
            .where(function () {
              this.where("branch_id", userDetails.branch_id).orWhere(
                "denomination_id",
  ...
  async viewSingleChurchBulletin({ params: { bulletin_id }, response, auth }) {
    try {
      ...
      const churchBulletins = await churchBulletinLog
        .query()
