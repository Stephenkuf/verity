...
class ChurchRequestController {
  async createChurchRequest({ request, response, auth }) {
    try {
      ...
      if (recipient == "branch") {
        const userBranch = await additionalUserInformation.findBy(
          "user_id",
          user.id
        );
        // console.log("KJKJK >>> ", userBranch);
        ...
        const branchInformation = await branches.findBy(
          "id",
      ...
      if (recipient == "denomination") {
        const userDenomination = await additionalUserInformation.findBy(
          "user_id",
          user.id
        );
        ...
        const denominationInformation = await denomination.findBy(
          "id",
          userDenomination.denomination_id
        );
  ...
  async viewChurchRequests({ request, response, auth }) {
    try {
      ...
      const churchRequests = await Database.table("request_users")
  ...
  async viewSingleChurchRequest({ params: { request_id }, response, auth }) {
    try {
      ...
      const verifyUser = await churchRequestUser
        .query()
        .where(function () {
      ...
      const churchRequests = await churchRequestUser
        .query()
        .where("id", request_id)
  ...
  async acceptChurchRequest({ request, response, auth }) {
    try {
      ...
      const userCheck = await churchRequestUser
        .query()
      ...
      const churchRequestAccept = await churchRequests
        .query()
        .where("id", request_id)
        .update({ is_accepted: 1, is_rejected: 0, reason: reason });
  ...
  async rejectChurchRequest({ request, response, auth }) {
    try {
      ...
      const churchRequestReject = await churchRequests
        .query()
        .where("id", request_id)
        .update({
          is_accepted: 0,
